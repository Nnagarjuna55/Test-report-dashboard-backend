...
export class FileController {
    constructor() {
        // MongoDB-based controller - no file system dependencies
    }

    public listDirectory = async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            const items = await FileService.getDirectoryContents(dirPath);
    ...
    public getFileContent = async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            const file = await FileService.getFileInfo(filePath);
            if (!file) {
                res.status(404).json({
                    success: false,
    ...
    public downloadItem = async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            const file = await FileService.getFileInfo(itemPath);
            if (!file) {
                res.status(404).json({
                    success: false,
            ...
            if (file.isFolder) {
                ...
                const stream = await FileService.getDownloadStream(itemPath);
                ...
                const content = await FileService.getFileContent(itemPath);
