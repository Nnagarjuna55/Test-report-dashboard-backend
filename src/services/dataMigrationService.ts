...
export class DataMigrationService {
    static async migrateFileSystemToMongoDB(dataDir: string): Promise<void> {
        try {
            ...
            await this.migrateDirectory(dataDir, '/');
    ...
    private static async migrateDirectory(fsPath: string, mongoPath: string): Promise<void> {
        try {
            ...
            if (stats.isDirectory()) {
                ...
                for (const item of contents) {
                    ...
                    await this.migrateDirectory(itemFsPath, itemMongoPath);
                }
            } else {
                // Create file entry in MongoDB
                const content = await this.readFileContent(fsPath);
                ...
                const file = new FileModel({
                    ...
                    metadata: {
                        ...
                        tags: this.generateTags(mongoPath),
                        author: 'Test System',
                        version: '1.0.0'
                    }
                });
